...
\subsubsection{Single pion production}
...
\subsubsection{Multi-$\pi$ and DIS}
...
The initial state description for all models but CCQE is the relativistic global Fermi gas \red{cite}

\subsubsection{Final State Interactions}
hadronic final state interactions are handled by a cascade implementation of the Salcedo-Oset model \red{cite}.
The pion FSI uncertainties come from fits to external pion-nucleus scattering data from a variety of experiments \red{find out which and cite}.

\subsubsection{Other interactions}
nue,numu 2\% on ratio and fully anti-correlated with anti-neutrino equivalent

NC1gamma (200\% nominal, 100\% uncertainty)

NCOther (NC res kaon, etc, NC DIS 30\%)

\subsection{Parameterisation}
The interaction parameters are parameterised as shapes or normalisations.
