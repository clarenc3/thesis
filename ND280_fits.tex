...
The raw Monte-Carlo event distributions with aforementioned binning is shown in \autoref{fig:nominal_mc}.
...
\end{subfigure}
\caption{Raw Monte-Carlo event distributions for the 6\numu, 4\numubar and 4\numu in RHC selections at ND280 in \{\pmu \cosmu\}}
\label{fig:nominal_mc}
\end{figure}

\subsection{Systematics}
The fit's main goal is to minimise impact of systematic parameters for T2K-SK oscillation analyses by using near-detector data. The shared parameters between the ND280 and SK are the neutrino flux parameters (since T2K and SK are in the same neutrino ``beamline''), and neutrino-nucleus interaction parameters. As such, there are a high number of ``parameters of interest'' to parameterise, which the following sections cover.
