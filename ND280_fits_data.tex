...
A number of alternative studies were performed for the 2017 analysis, comparing subruns in data and MC, change of model parameters and their priors, investigating effects of removing runs, and comparing FGD1 and FGD2. The largest impact was found in the FHC versus RHC data fit, and the FGD1 vs FGD2 data fit, with some parameters outside the $1\sigma$ range of the full fit to data. The parameter values of these two fits---and a fit using a 2015-like model with BeRPA and 2p2h shape fixed---were propagated to Super-Kamiokande and event rate comparisons were made. Generally, there was negligible ($<1\sigma$) differences even when parameter values were different, and the studies were all compatible with respect to impact on oscillation analyses. These studies are found in \autoref{sec:data_alt_studies}.

\section{Cross-group validation}
To further validate the implementation and results of the 2017 ND280-only fits before the full oscillation analysis was run, comparisons are made to the frequentist ``BANFF'' group. This group gives information on systematics for oscillation analyses by providing central values and uncertainties related through a covariance matrix which are marginalised over. The validation saw differences in all of the flux parameters, and some of the interaction parameter all due to marginalisation effects when providing the point estimates from the MCMC. However, the posterior predictive distributions compared to the ``best-fit'' distributions agreed well and validation tests were all complete. \autoref{chap:banff_valid} details this procedure.